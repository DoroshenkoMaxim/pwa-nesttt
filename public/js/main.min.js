class PWAInstaller {
  constructor(debug = false) {
    ...
    this.status = "none"; // Возможные состояния: "none", "ready", "installing", "installed"
    this.debugLog("PWAInstaller initialized");
  }

  // Метод для логирования, если включен debug
  debugLog(message) {
    if (this.debug) {
      console.log(`[PWAInstaller] ${message}`);
    }
  }
  ...
  onBeforeInstallPrompt(event) {
    ...
    this.status = "ready";
    this.debugLog("Install prompt ready");
  }

  // Показываем диалог установки
  async showInstallPrompt() {
    if (!this.deferredPrompt) {
      this.debugLog("Install prompt not available");
      return;
    }
    ...
    if (outcome === "accepted") {
      this.debugLog("User accepted the install prompt");
      this.status = "installed";
    } else {
      this.debugLog("User dismissed the install prompt");
      this.status = "none";
    }

    this.deferredPrompt = null; // Сбрасываем deferredPrompt
  }

  // Проверяем, установлено ли PWA
  async checkIfInstalled(callback) {
    if ("getInstalledRelatedApps" in navigator) {
      const relatedApps = await navigator.getInstalledRelatedApps();
      if (relatedApps.length > 0) {
        this.status = "installed";
        this.debugLog("PWA is already installed");
        if (callback) callback();
      }
    }
  }

  // Инициализация событий для PWA
  init() {
    window.addEventListener("beforeinstallprompt", (event) => this.onBeforeInstallPrompt(event));
    window.addEventListener("appinstalled", () => {
      this.status = "installed";
      this.debugLog("PWA installed event triggered");
    });
  }
}
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  if (app.displayMode !== "standalone") {
    ...
    window.addEventListener("serviceWorkerRegistration", () => {
      ...
      checkInstalled();
      const intervalId = setInterval(checkInstalled, 1);
